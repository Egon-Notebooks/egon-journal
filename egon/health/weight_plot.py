...
def plot_weight(
    ...
    if lean_data:
        ...
        ax_lean.plot(lean_dates, lean_values, color="#DD8452", linewidth=1.5,
                     alpha=0.9)
        ax_lean.scatter(lean_dates, lean_values, color="#DD8452", s=18,
                        zorder=3, alpha=0.7)
        ax_lean.axhline(lean_avg, color="#DD8452", linewidth=0.9,
                        linestyle="--", alpha=0.5,
                        label=f"avg {lean_avg:.1f} {unit}")
        if target_lean_body_mass is not None:
            ax_lean.axhline(target_lean_body_mass, color="#DD8452",
